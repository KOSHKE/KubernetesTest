...
func toPBPayment(p *dto.PaymentResponse) *pb.Payment {
	...
	return &pb.Payment{
		...
		OrderId:       p.OrderID,
		UserId:        p.UserID,
		Amount:        &common.Money{Amount: p.AmountAmount, Currency: p.AmountCurrency},
...
func (s *PBPaymentServer) ProcessPayment(ctx context.Context, req *pb.ProcessPaymentRequest) (*pb.ProcessPaymentResponse, error) {
	...
	processReq := &dto.ProcessPaymentRequest{
		...
		OrderID:        req.OrderId,
		UserID:         req.UserId,
		AmountAmount:   amt.Amount,
		AmountCurrency: amt.Currency.Code,
	...
	// Build response
	success := resp.Status == string(paymentvalueobjects.PaymentStatusCompleted)
