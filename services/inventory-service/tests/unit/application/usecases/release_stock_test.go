...
// setupReleaseStockTest creates common test setup for ReleaseStockUseCase tests
func setupReleaseStockTest(t *testing.T) (*gomock.Controller, *mocks.MockInventoryRepositoryFacade, *usecases.ReleaseStockUseCase) {
	ctrl := gomock.NewController(t)
	mockRepo := mocks.NewMockInventoryRepositoryFacade(ctrl)
	useCase := usecases.NewReleaseStockUseCase()
...
func TestReleaseStockUseCase_Execute(t *testing.T) {
	t.Run("success", func(t *testing.T) {
		t.Parallel()
		// Arrange
		ctrl, mockRepo, useCase := setupReleaseStockTest(t)
		defer ctrl.Finish()
		...
		mockRepo.EXPECT().
			...
			DoAndReturn(func(ctx context.Context, stocks []*entities.Stock) error {
				// Verify that stocks were updated with released quantities
