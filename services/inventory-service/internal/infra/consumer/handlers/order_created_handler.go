...
func (h *OrderCreatedHandler) Handle(ctx context.Context, evt *events.OrderCreated) error {
	...
	// Reserve stock
	success, failedProducts, err := h.domainService.ReserveStock(ctx, evt.OrderId, evt.UserId, items)
	if err != nil {
	...
	if !success {
		h.logger.Warn("stock reservation failed", "orderID", evt.OrderId, "failedProducts", failedProducts)
	}
